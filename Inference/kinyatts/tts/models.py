...
from kinyatts.tts.attentions import Encoder, FFT
from kinyatts.tts.commons import sequence_mask, rand_slice_segments, generate_path
from kinyatts.tts.modules import Log, ElementwiseAffine, ConvFlow, Flip, DDSConv, LayerNorm, WN, \
    ResidualCouplingLayer, ResBlock1, ResBlock2, LRELU_SLOPE
from kinyatts.tts.stft import TorchSTFT, OnnxSTFT
from monotonic_align import maximum_path
from kinyatts.tts.commons import init_weights, get_padding
from kinyatts.tts.pqmf import PQMF
  ...
  """
    ...
    def forward(self, x, x_lengths, y, y_lengths):
        ...
        if self.n_speakers > 0:
            g = self.emb_g(torch.LongTensor([0]).to(x.device)).unsqueeze(-1)  # [b, h, 1]
        else:
            g = None
    ...
    def infer(self, x, x_lengths, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
        if self.n_speakers > 0:
            g = self.emb_g(torch.LongTensor([0]).to(x.device)).unsqueeze(-1)  # [b, h, 1]
        else:
            g = None
