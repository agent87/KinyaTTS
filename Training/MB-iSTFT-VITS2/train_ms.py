    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(loader):
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                losses_names = ['loss_disc', 'loss_gen', 'loss_fm', 'loss_mel', 'loss_dur', 'loss_kl', 'loss_subband']
                losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_dur, loss_kl, loss_subband]
                logger.info('Train Epoch: {} [{:.0f}%]'.format(
                    epoch,
                    100. * batch_idx / len(train_loader)))
                milli_steps_per_second = 1000.0 * (global_step - start_steps) / (time.time() - start_time)
                logger.info(f' MSPS: {milli_steps_per_second:.3f} ' + time_now() + ' ' + (' '.join([f'{y} {x.item():.6f}' for x,y in zip(losses,losses_names)] + ['Steps', str(global_step), f'LR {lr:.6f}'])))
